...
public class QuickSort {
	...
	public static void qsort(int[] array,int start,int end){
		if(start<end){
			int partition_key=hPartition(array,start,end);
			printArray(array);
	...
	private static int hPartition(int[] array,int start,int end){
		int pivot = array[start];
		int i = start - 1;
		int j = end + 1;
		System.out.println("Partition called with start="+start+" end="+end);
		while(true){
			while(array[--j] > pivot);
			while(array[++i] < pivot);
			if(i>=j){
